...
def clean_data(df):
    logger.info("Cleaning data: dropping nulls and duplicates")
...
def main():
    ...
    # Match date with DAG run (which uses current date as partition)
    today_date = datetime.now().strftime('%Y-%m-%d')

    input_path = f"gs://stock-bucket8/stock_data/date={today_date}/*.json"
    output_path = f"gs://stock-bucket8/stock_transformed/date={today_date}/"

    df_raw = read_data(spark, input_path)
    if df_raw.rdd.isEmpty():
        logger.warning(f"No data found at input path: {input_path}")
    else:
